...
Then, neglecting the input saturation effect, the steady-state error bound between the system and the desired system can be reduced by minimizing the condition number $\chi$ of the contraction metric $\mm{M}$ proportionally, as follows:
...
\begin{equation} \label{eq:retrive:mm}
   ...
   \mm{I}_n
   -
   \frac{\mv{e}\mv{e}^\top}{\mv{e}^\top\mv{e}}
   +
...
Moreover, if $\mv{y}^\top\mv{e}>0$, then $\mm{M}\succ0$.
...
Consequently, the time variation of $\nu$ caused the resulting metric to violate the contraction constraint \eqref{eq:cstr:ctrc}, as shown in Fig.~\ref{fig:sim:result:opt}.
