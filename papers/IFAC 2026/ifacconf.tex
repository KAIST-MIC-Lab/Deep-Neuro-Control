...
$
   ...
   ,
$
where $\overline{\mv{f}}:=\mv{f}-\mv{f}_d+(\mm{B}-\mm{B}_d)\mv{u}_d$, the error dynamics \eqref{eq:error:dyn:1} can be linearized as follows:
...
This makes the optimization problem \eqref{eq:opt:conv:lmi} potentially infeasible when the input saturation constraint is too conservative.
...
Section \ref{sec:sub:vector:space:opt} presents the proposed vector-space optimization framework for contraction metric calculation, which resolves the issues in the conventional LMI framework discussed in Section \ref{sec:conv:lmi}.
...
\begin{equation} \label{eq:obj:angle}
   \norm{\mv{y}}\norm{\mv{e}} - \mv{y}^\top \mv{e}
   ,
\end{equation}
which is minimized when the angle between $\mv{y}$ and $\mv{e}$ is zero.
...
To this end, an energy-based constraint is introduced to obtain a locally optimal solutions while maintaining sufficient control effort, as follows:
...
If the initial guess is set near point (a), the optimal solution converges to $\mv{y}_a$, point (a), neglecting the input saturation constraint according to the objective function, \ie the minimum angle $\theta_a$ between $\mv{y}$ and $\mv{e}$.
Considering the input saturation constraint, the optimal solution is obtained at $\mv{y}_b$, point (b), resulting in angle $\theta_b$, by increasing the slack variable $s$, which relaxes the energy-based constraint \eqref{eq:cstr:energy} to satisfy the input saturation constraint.
In this manner, the condition number of the contraction metric can be increased when the contraction constraint \eqref{eq:cstr:ctrc:proj} or the input saturation constraint \eqref{eq:cstr:sat} conflict.
