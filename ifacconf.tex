...
\begin{equation} \label{eq:model:SM}
   ...
   \end{cases}
\end{equation}
where $\theta\in\R$ is the rotor angle, $\omega\in\R$ is the rotor speed, $J\in\R_{>0}$ is the moment of inertia, $\tau:=\tau(\ia,\ib)\in\R^2$ is the electromagnetic torque generated by the machine, $f_v$ is the viscous friction coefficient, and $\tau_L$ is the load torque.
The electromagnetic torque $\tau$ is typically a function of the phase currents $\ia$ and $\ib$ of the machine, which can be expressed as:
\begin{equation} \label{eq:tau:SM}
   ...
   ,
\end{equation}
where $P$ is the number of pole pairs and $\Phi$ is the flux linkage.
...
\begin{equation} \label{eq:model:SM:state}
   ...
   ,
\end{equation}
where $\mv{x} := \left(\theta, \omega\right)^\top$, $\mv{u} := \left(\ia, \ib\right)^\top$ and $d := \tau_L$ denote the state vector, the control input vector and the disturbance vector, respectively.
The system matrix $\mm{A}$ and the input matrices $\mm{B}_1:=\mm{B}_1(\mv{x})$ and $\mm{B}_2$ are defined as follows:
\begin{equation}
   \mm{A} =
   \begin{bmatrix}
      0 & 1 \\
...
The contraction property of the system \eqref{eq:sys} can be analyzed using the Lyapunov function $V:=V(\mv{x},t)=\delta\mv{x}^\top \mm{M} \delta\mv{x}$, where $\mm{M} := \mm{M}(\mv{x},t)$ is a positive definite matrix called the \textit{contraction metric}.
...
\begin{equation} \label{eq:contraction:condition}
   ...
   ,
\end{equation}
which leads to $\ddtt V \leq -\alpha V$ for some $\alpha > 0$.
...
\begin{thm} [Thm.~1 \cite{Wang:2004aa}] \label{thm:partial:contraction}
