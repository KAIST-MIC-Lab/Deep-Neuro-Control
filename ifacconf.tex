...
\begin{equation}
   \begin{matrix}
      ...
      % \begin{cases}
         ...
         -
         2\mu\mm{B}\mm{R}^{-1} \mm{B}^\top 
         \preceq
         -2\alpha\overline{\mm{W}}
         ,
         % \\
         % &
         % \overline{\mm{W}} = \overline{\mm{W}}^\top \succ 0
         % ,
      % \end{cases}
   \end{matrix}
\end{equation} 
where $\overline{\mm{W}}:=\mu\mm{W}$ and $\mu\in\R_{>0}$ is a normalizing term of $\mm{M}$.
...
\begin{equation} \label{eq:cstr:energy}
   \left(
      \mv{e}^\top \mv{y} 
      -
      m_{\rm{energy}}\mv{e}^\top \mv{e}
   \right)^2
   \le
   (\epsilon + s) \norm{\mv{e}}^2
   ,
\end{equation}
where $m_{\rm{energy}}\in\R_{>0}$ denotes the desired energy level, and $\epsilon\in\R_{>0}$ and $s\in\R_{\ge0}$ denote allowable energy variation and a slack variable to enhance the robust feasibility of the optimization problem.

In Fig.~\ref{fig:scene:first}, the constraints are illustrated in two-dimensional space for better intuition.
The outside of blue eclipse and inside of red eclipse represent the feasible space of the contraction constraint \eqref{eq:cstr:ctrc:proj} and the input saturation constraint $\mysat(\mv{y})\le0$, respectively.
...
\begin{table}[t]
   ...
   \hline
   (C$_3$) \protect\colorLine{gray}{solid} & Desired controller \\
   \hline
   \end{tabular}
...
\begin{figure*}[t]
   ...
   \caption{
   Numerical validation results of (C$_1$) \protect\colorLine{blue}{solid}, (C$_2$) \protect\colorLine{my_cyan}{solid}, (C$_3$) \protect\colorLine{gray}{solid}, (C$_4$) \protect\colorLine{magenta}{solid}, and desired trajectory $x_d$ \protect\colorLine{red}{dashed}.
}
\label{fig:result}
\end{figure*}
