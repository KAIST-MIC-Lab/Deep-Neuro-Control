...
\begin{equation} \label{eq:sys}
   ...
   .
\end{equation}
The contraction theory commences with the definition of \textit{differential dynamics} of the system \eqref{eq:sys} using \textit{differential displacement} $\delta\mv{x}$ \cite[Chap 4]{Kirk:2004aa}, which is defined as follows:
\begin{equation}
   \ddtt{\delta\mv{x}}
   =
...
The contraction property of the system \eqref{eq:sys} can be analyzed using the Lyapunov function $V:=V(\mv{x},t)=\tfrac{1}{2}\delta\mv{x}^\top \mm{M} \delta\mv{x}$, where $\mm{M}:=\mm{M}(\mv{x},\mv{x}_d,t)\in\R^{n\times n}$ the \textit{contraction metric}.
