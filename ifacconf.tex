...
For your intuition, the following example illustrates the application of the partial contraction theory.
...
Motivated by the use of the partial contraction theory in \cite{Tsukamoto:2021af}, \MSRY{and more...}, we assume that the desired trajectory (including desired control input) of the system is given without disturbances, \ie $d=0$, as follows:
\begin{equation} \label{eq:desired:sys}
   ...
   ,
\end{equation}
where $\mv{x}^*\in\R^2$ is the desired state vector and $\mv{u}^*\in\R^2$ is the desired control input vector.
...
\begin{equation} 
   ...
   +
   2\mm{M}\mm{B}_1\mm{R}^{-1} \mm{B}_1^\top \mm{M}
   \leq
   -2\alpha\mm{M}
\end{equation}
