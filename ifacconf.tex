...
\begin{equation} \label{eq:sys}
   ...
   .
\end{equation}
The contraction theory \cred{starts} with the definition of \textit{differential dynamics} of the system \eqref{eq:sys} using \textit{differential displacement} $\delta\mv{x}$ \cite[Chap 4]{Kirk:2004aa}, which is defined as follows:
\begin{equation}
   \ddtt{\delta\mv{x}}
   =
...
The contraction property of the system \eqref{eq:sys} can be analyzed using the Lyapunov function $V:=V(\mv{x},t)=\tfrac{1}{2}\delta\mv{x}^\top \mm{M} \delta\mv{x}$, where $\mm{M}:=\mm{M}(\mv{x},\mv{x}_d,t)\in\R^{n\times n}$ is a symmetric positive defionite matr

$\overline{m}\mm{I}\succeq\mm{M}(\mv{x},\mv{x_d},t)=\mm{M}^\top\succeq\underline{m}\mm{I}$ with $\overline{m},\underline{m}\in\R_{>0}$ denotes the \textit{contraction metric}.
...
\subsection{Partial Contraction Theory}
...
\begin{pf}
   \cred{
   Assume that the possible solutions of the virtual system $\mv{q}$ are $\mv{\xi}_1(t)$ and $\mv{\xi}_2(t)$ which are the trajectories of the nonlinear system $\mv{x}$.
   ...
   }
\end{pf}

\subsection{Perturbed Contracting System}
...
\begin{equation} \label{eq:sys:target}
   \begin{aligned}   
      \ddtt{\mv{x}}
      =&
      \mv{f}(\mv{x},t)
...
\begin{equation} \label{eq:control:sat}
   \mysat(\mv{u}) =
   \begin{cases}
...
The difference between the actual control input $\mv{u}$ and the saturated control input is denoted by $\mv{\phi}(\mv{u}) = \mysat(\mv{u}) - \mv{u}$.
...
Motivated by the use of the partial contraction theory in \cite{Tsukamoto:2021af}, \MSRY{and more...}, we assume that the desired trajectory (including desired unsaturated control input, \ie $\norm{\mv{u}_d}\le\overline{u}$) of the system is given without disturbances, \ie $d=0$, as follows:
\begin{equation} \label{eq:desired:sys}
   ...
   ,
\end{equation}
where $\mv{x}_d\in\R^n$ is the desired state vector and $\mv{u}_d\in\R^n$ is the desired control input vector, and $\mv{f}_d:=\mv{f}(\mv{x}_d, t)$ and $\mm{B}_d:=\mm{B}(\mv{x}_d,t)$.

Finally, the control objective is to design a control input $\mv{u}$ such that the system trajectories of the system \eqref{eq:desired:sys} and the desired system \eqref{eq:desired:sys} converges, under the existences of the disturbances $\mv{d}$ and input saturation $\mysat(\cdot)$.
...
$
   ...
   - \mm{B}_d \mv{u}_d
$
and $\mm{B}_d := \mm{B}(\mv{x}_d)$, and $c$ is a dummy variable for the integration.
...
\end{equation}
where $\mm{R} \succ 0$ is a positive definite matrix and $\mm{M}:=\mm{M}(\mv{x},\mv{x_d},t) \succ 0$ is the contraction metric defined in Section \ref{sec:sub:contraction}.

\subsection{Contraction Metric Calculation}

\subsubsection{Contraction Condition Constraint}

Invoking the partial contraction theory reviewed in Section \ref{sec:sub:contraction}, we can define the virtual system as follows:
...
Then according to Theorem \ref{thm:contraction:perturbed} and the partial contraction theory, the system \eqref{eq:sys:virtual} and the system \eqref{eq:desired:sys} will converge to the desired trajectory $\mv{x}_d$ with steady state error:
...
To address the input saturation function \eqref{eq:control:sat}, the saturation constraint $\mv{u}^\top \mv{u} - \overline{u}^2 \le 0$ is defined as:
\begin{equation}
   \begin{bmatrix}
...
\subsection{Scenario 1: Contraction Property}
...
\begin{figure*}[t]
   ...
   \caption{
   Real-time implementation results of (C$_1$) \protect\colorLine{blue}{solid}, (C$_2$) \protect\colorLine{cyan}{solid}, (C$_3$) \protect\colorLine{gray}{solid}, (C$_4$) \protect\colorLine{magenta}{solid}, and desired trajectory $x_d$ \protect\colorLine{red}{dashed}.
}
\label{fig:ctrl:result}
\end{figure*}


\subsection{Scenario 2: Saturation Handling}
