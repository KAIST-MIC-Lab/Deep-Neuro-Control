...
\begin{abstract}                % Abstract of 50--100 words
   ...
   It allows us to analyze the stability of systems by examining the convergence of trajectories.
   The recent development of contraction theory based control has introduced computational efficient methods for obtaining contraction metrics using linear matrix inequalities (LMIs).
   ...
   The proposed method is demonstrated its effectiveness through numerical simulations using \cred{Lorenz system}.
\end{abstract}
...
Hence, the main objective of this paper is to extend the contraction theory based control strategy to account for input saturation simultaneously.
...
\begin{itemize}
   ...
   \item ...; and
   \item The effectiveness and the feasibility of the contraction theory based control is validated in Lorenz system.
\end{itemize}
...
The contraction property of the system \eqref{eq:sys} can be analyzed using the Lyapunov function $V:=V(\mv{x},t)=\tfrac{1}{2}\delta\mv{x}^\top \mm{M} \delta\mv{x}$, where $\overline{m}\mm{I}\succeq\mm{M}(\mv{x},\mv{x_d},t)=\mm{M}^\top\succeq\underline{m}\mm{I}$ with $\overline{m},\underline{m}\in\R_{>0}$ denotes the \textit{contraction metric}.
...
\section{Contraction Theory Based Control}
...
\begin{equation}
   ...
   ,
\end{equation}
where $\sigma=10$, $\rho=28$, and $\beta=8/3$ are system parameters.
