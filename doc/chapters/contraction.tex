...
\section{Review of Contraction Theory}\label{chap:contraction}
...
The smooth property of $\bdf(\bdx,t)$ is essential to ensure the existence and uniqueness of the solution to \eqref{eq:ctrac:sys} \cite[see, pp. 88-89]{Khalil:2002aa}.
...
For this, motivated by the calculus of variations \cite[Chap. 4]{Kirk:2004aa}, \eqref{eq:ctrac:sys} can be rewritten as differential dynamics using \textit{differential displacement} $\delta\bdx$ as follows:
\begin{equation}
    \ddtt\delta\bdx
    =
    \tpfpx(\bdx,t)
...
\begin{definition}[see, Def. 2.2 \cite{Tsukamoto:2021aa}]  
    If any two trajectories $\mv\xi_1(t)$ and $\mv\xi_2(t)$ of \eqref{eq:ctrac:sys} converge to a single trajectory, then the system \eqref{eq:ctrac:sys} is said to be \textit{incrementally exponentially stable}, if $\exists C,\alpha>0$, subject to the following holds:
    \begin{equation}      
...
\begin{definition}
    ...
    Then, a symmetric continuously differentiable matrix $\mm M(\bdx,t):=\mm\Theta(\bdx,t)^\top\mm\Theta(\bdx,t)$ is said to be a \textit{metric} of the system \eqref{eq:ctrac:sys}.
    \label{def:metric}
\end{definition}
...
The following theorem presents the fundamental theorem of contraction theory and corresponding necessary and sufficient condition for exponential convergence of the differential system \eqref{eq:ctrac:diff_sys}.
...
\begin{theorem}[see, T. 2.1 \cite{Tsukamoto:2021aa}]
    ...
    \begin{subequations}
        ...
        \label{eq:ctrac:ctrac:metric:F}
            ...
            -2\alpha\mm M
            ,
        \label{eq:ctrac:ctrac:metric:M}
        \end{align}
        \label{eq:ctrac:ctrac:metric}
    \end{subequations}
    where the arguments of $\mm M(\bdx,t)$ and $\mm F(\bdx,t)$ are omitted for simplicity, then, the system \eqref{eq:ctrac:sys} is said to be contracting with an exponential rate $\alpha$, \ie all trajectories of \eqref{eq:ctrac:sys} converge to a single trajectory.
    The converse is also true.
    \label{thm:ctrac:main}
\end{theorem}

\begin{proof}
    Taking time derivative of a differential Lyapunov function of $\delta\bdx$, $V=\delta\bdz^\top\bdz=\delta\bdx^\top\mm M\delta\bdx$, using the differential dynamics \eqref{eq:ctrac:diff_sys}, we have
    \begin{equation}
        \begin{aligned}
...
Theorem \ref{thm:ctrac:main} can also be proven by using the transformed squared length integrated over two arbitrary solutions of \eqref{eq:ctrac:sys}.
...
\begin{theorem}[see, p. 688 \cite{LOHMILLER:1998aa}, T. 2.3 \cite{Tsukamoto:2021aa}]
    Let $\mv{\xi}_1(t)$ and $\mv{\xi}_2(t)$ be two solutions of \eqref{eq:ctrac:sys}, and define the transformed squared length with $\mm M(\bdx,t)$ of Theorem \ref{thm:ctrac:main} as follows:
    \begin{equation}
    ...
    \begin{equation}
        ...
        \label{eq:ctrac:Vl}
    \end{equation}
    Then, \eqref{eq:ctrac:Vsl} and \eqref{eq:ctrac:Vl} satisfy the following inequality:
    \begin{equation}
        \norm{\mv\xi_1(t)-\mv\xi_2(t)}
        =
        \norm{
        ...
        ,
        \label{eq:ctrac:xi_x_Vl_Vsl}
    \end{equation}
    where $\mm M(\bdx,t) \ge \underbar{m}\mm I_n,\ \forall \bdx, t$ for $\exists\underbar{m}\in\R_{>0}$ and Theorem \ref{thm:ctrac:main} can also be proven by using \eqref{eq:ctrac:Vsl} and \eqref{eq:ctrac:Vl} as a Lyapunov-like function, resulting in incremental exponential stability of the system \eqref{eq:ctrac:sys} (see, Definition \ref{def:inc_exp_stable}).
    Note that the shortest path integral $V_l$ of \eqref{eq:ctrac:Vl} with a parameterized state $\bdx$ (\ie $\inf{V_l}=\sqrt{\inf{V_{sl}}}$) defines the Riemannian distance and the path integral of a minimizing geodesic.
    \label{thm:ctrac:path_int}
\end{theorem}

\begin{proof}
    Note that $\mv{\xi}_i(t),\ \forall i\in\{1,2\}$ are parameterized $\bdx$.
    Therefore, left-side equailty of \eqref{eq:ctrac:xi_x_Vl_Vsl} holds.
    The right-side inequality of \eqref{eq:ctrac:xi_x_Vl_Vsl} can be proven by using the Cauchy-Schwarz inequality.
    ...
    As $\mm{M}(\bdx,t)$ is from Theorem \ref{thm:ctrac:main}, the incremental exponential stability of the system \eqref{eq:ctrac:sys} is guaranteed using the comparison lemma of Lemma \ref{lem:comparison} as follows: 
    \begin{equation}
        \norm{\mv\xi_1(t)-\mv\xi_2(t)}
        \le
...
In conclusion, Theorem \ref{thm:ctrac:main} defines necessary and sufficient conditions of incremental exponential stability of the system \eqref{eq:ctrac:sys}, and Theorem \ref{thm:ctrac:path_int} provides an alternative proof of Theorem \ref{thm:ctrac:main} using the path integral of a minimizing geodesic.
...
\begin{example}[see, Ex. 2.2 \cite{Tsukamoto:2021aa}]
    ...
    \begin{equation}
        ...
        .
    \end{equation}
    The differential dynamics of the system can be rewritten as \eqref{eq:ctrac:diff_sys} with $\delta\bdx=\delta x$ as follows:
    \begin{equation}
        \ddtt\delta x
        =
        -\delta x
        .
    \end{equation}
    Using constraint \eqref{eq:ctrac:ctrac:metric:M}, one can select the metric $\mm M=I$.
    Then, one can conclude that the system \eqref{eq:ctrac:sys} is contracting with an exponential rate $\alpha=1$.
    However, the system \eqref{eq:ctrac:sys} is not stable, since the solution of the system \eqref{eq:ctrac:sys} is $x(t) = \tfrac{\exp(t)}{2}+(x(0)-\tfrac{1}{2})\exp(-t)$.
    \label{ex:div}
\end{example}
...
\begin{example}[see, Ex. 1 \cite{Jouffroy:2004aa}]
    ...
    The differential dynamics of the system can be rewritten as \eqref{eq:ctrac:diff_sys} with $\delta\bdx=\begin{bmatrix}\delta x_1 & \delta x_2\end{bmatrix}^\top$ as follows:
    \begin{equation}
        \ddtt
        \begin{bmatrix}
            \delta x_1\\
...
Let us define deterministic prerturbated system by adding a bounded perturbation $\bdd(\bdx,t)$ to the system \eqref{eq:ctrac:sys} as follows:
\begin{equation}
    \ddtt\bdx
    =
    \bdf(\bdx,t)
    ...
    \bdd(\bdx,t)
    ,
    \label{eq:ctrac:sys:pert}
\end{equation}
and two solutions of \eqref{eq:ctrac:sys} $\mv\xi_1(t)$ and \eqref{eq:ctrac:sys:pert} $\mv\xi_2(t)$.
...
\begin{equation}
    ...
    ,
    \label{eq:ctrac:sys:pert:virtual}
\end{equation}
...
\begin{theorem}[see, Eq. 15 \cite{LOHMILLER:1998aa}, Lemma 1 \cite{Dani:2015aa}, T. 2.4 \cite{Tsukamoto:2021aa}]
    If the system \eqref{eq:ctrac:sys} satisfies \eqref{eq:ctrac:ctrac:metric} of Theorem \ref{thm:ctrac:main}, then the path integral of the virtual system \eqref{eq:ctrac:sys:pert:virtual} (\ie $V_l(\bdq,\delta\bdq,t)=\int_{\mv\xi_1}^{\mv\xi_2}\norm{\mm\Theta(\bdq,t)\delta\bdq}$), where $\mv{\xi}_1(t)$ and $\mv{\xi}_2(t)$ are solutions of \eqref{eq:ctrac:sys} and \eqref{eq:ctrac:sys:pert}, respectively, converges to a bounded error ball as long as $\mm\Theta\bdd\in \mathcal{L}_\infty$.
    Specifically, if $\exists \underbar m, \overline m\in\R_{>0}$ such that $\underbar m\mm I_n\le\mm M(\bdx,t)\le\overline m\mm I_n$, and $\overline d = \sup_{\bdx,t} \norm{\bdd(\bdx,t)}$, then the following inequality holds:
    \begin{equation}
        \norm{\mv\xi_1(t)-\mv\xi_2(t)}
        \le
...
\begin{proof}
    ...
    \begin{equation}
        ...
        .
    \end{equation}
    By using the inequality \eqref{eq:ctrac:xi_x_Vl_Vsl}, we have the desired result as follows:
    \begin{equation}
        \norm{\mv\xi_1(t)-\mv\xi_2(t)}
        \le
        ...
        ,
    \end{equation}
    where $\underbar m=\inf_t\lambda_{\min}(\mm M)$ and $\overline m=\sup_t\lambda_{\max}(\mm M)$.
